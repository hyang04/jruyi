...
public final class CharsetCodecProvider implements IFactory {
	...
	private static final CharsetCodecProvider m_inst = new CharsetCodecProvider();
	private static final Map<String, SoftReference<CharsetCodec>> m_cache = new ConcurrentHashMap<String, SoftReference<CharsetCodec>>();
				...
				for (i = offset; i < j; ++i) {
					...
					if (length > 0) {
						if (n > 0) {
							...
							cb = builder.getCharBuffer(0, length).put(cb);
							builder.setLength(length);
						}
					} else
						builder.setLength(0);
				...
				for (i = offset; i < j; ++i) {
					...
					if (length > 0) {
						if (n > 0) {
							...
							builder.ensureCapacity(length);
							bb = builder.getByteBuffer(0, length).put(bb);
							builder.setLength(length);
						}
					} else
						builder.setLength(0);
	...
	static final class DefaultCharsetCodecHolder {
		...
		static final CharsetCodec m_defaultCharsetCodec = new CharsetCodec(
	...
	public ICharsetCodec get() {
		return DefaultCharsetCodecHolder.m_defaultCharsetCodec;
	}
	...
	public ICharsetCodec get(String charsetName) {
		...
		SoftReference<CharsetCodec> reference = m_cache.get(charsetName);
		CharsetCodec cc = reference != null ? reference.get() : null;
		if (cc == null) {
			Map<String, SoftReference<CharsetCodec>> cache = m_cache;
	...
	public ICharsetCodec get(Charset charset) {
		...
		SoftReference<CharsetCodec> reference = m_cache.get(name);
