...
public final class StringBuilder implements Serializable, Appendable,
	...
	private static final char[] m_cs = StrUtil.getLineSeparator().toCharArray();
	private static final char[] m_bhDigits = new char[256];
	private static final char[] m_blDigits = new char[256];
	private static final IThreadLocalCache<StringBuilder> m_cache = ThreadLocalCache
			.weakArrayCache();
	...
	static {
		for (int i = 0; i < 256; ++i) {
			int hex = i >>> 4;
			hex += (hex < 10 ? '0' : ('A' - 10));
			m_bhDigits[i] = (char) hex;

			hex = i & 0x0F;
			hex += (hex < 10 ? '0' : ('A' - 10));
			m_blDigits[i] = (char) hex;
		}
	}
	...
	public static StringBuilder get() {
		StringBuilder builder = m_cache.take();
		if (builder == null)
	...
	public StringBuilder appendHex(byte b) {
		...
		int bt = b & 0xFF;
		m_value[length] = m_bhDigits[bt];
		m_value[length + 1] = m_blDigits[bt];
		m_length = newLength;
	...
	public StringBuilder appendHex(short s) {
		...
		newLength = m_length;
		int b = (s >> 8) & 0xFF;
		value[newLength] = m_bhDigits[b];
		value[++newLength] = m_blDigits[b];
	...
	public StringBuilder appendHex(long l) {
		...
		for (int bits = 24; bits >= 0; bits -= 8, ++c1, ++c2) {
			int b = (i1 >> bits) & 0xFF;
			value[c1] = m_bhDigits[b];
			value[++c1] = m_blDigits[b];

			b = (i2 >> bits) & 0xFF;
			value[c2] = m_bhDigits[b];
			value[++c2] = m_blDigits[b];
		}
	...
	public StringBuilder appendHexDump(byte[] b, int offset, int length) {
		...
		char[] v = m_value;
		char[] bhDigits = m_bhDigits;
		char[] blDigits = m_blDigits;

		System.arraycopy(m_cs, 0, v, n, LSL);
		n += LSL;
		...
		while (length > 0) {
			...
			System.arraycopy(m_cs, 0, v, ++i, LSL);
			n = i + LSL;
	...
	public StringBuilder insertHex(int offset, byte b) {
		...
		char[] v = m_value;
		int bt = b & 0xFF;
		v[offset] = m_bhDigits[bt];
		v[++offset] = m_blDigits[bt];
	...
	public StringBuilder insertHex(int offset, short s) {
		...
		char[] v = m_value;
		int b = (s >> 8) & 0xFF;
		v[offset] = m_bhDigits[b];
		v[++offset] = m_blDigits[b];
	...
	public StringBuilder insertHex(int offset, long l) {
		...
		for (int bits = 24; bits >= 0; bits -= 8, ++offset, ++offset2) {
			...
			b = (i2 >> bits) & 0xFF;
			v[offset2] = m_bhDigits[b];
			v[++offset2] = m_blDigits[b];
		}
	 ...
	 * released. So the reference to this object must not be used anymore after
	...
	void appendHexDump(byte[][] data, int[] offsets, int[] lengths, int size) {
		...
		do {
			...
			System.arraycopy(m_cs, 0, v, ++asc, LSL);
			n = asc + LSL;
