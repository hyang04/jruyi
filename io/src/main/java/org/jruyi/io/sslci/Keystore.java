...
public final class Keystore implements ISslContextInfo {
	...
	static final class Configuration {
		...
		static final Method[] m_keyProps;
		static final Method[] m_srProps;
		static final Method[] m_trustProps;
		...
		static {
			...
			try {
				m_keyProps = new Method[KEYPROPS.length];
				for (int i = 0; i < KEYPROPS.length; ++i)
					m_keyProps[i] = clazz.getMethod(KEYPROPS[i]);

				m_trustProps = new Method[TRUSTPROPS.length];
				for (int i = 0; i < TRUSTPROPS.length; ++i)
					m_trustProps[i] = clazz.getMethod(TRUSTPROPS[i]);

				m_srProps = new Method[SRPROPS.length];
				for (int i = 0; i < SRPROPS.length; ++i)
					m_srProps[i] = clazz.getMethod(SRPROPS[i]);
	...
	protected void modified(Map<String, ?> properties) throws Exception {
		...
		KeyManager[] keyManagers = newConf.isChanged(conf,
				Configuration.m_keyProps) ? getKeyManagers(newConf)
				: m_keyManagers;
		TrustManager[] trustManagers = newConf.isChanged(conf,
				Configuration.m_trustProps) ? getTrustManagers(newConf)
				: m_trustManagers;
		SecureRandom secureRandom = newConf.isChanged(conf,
				Configuration.m_srProps) ? getSecureRandom(newConf)
				: m_secureRandom;
