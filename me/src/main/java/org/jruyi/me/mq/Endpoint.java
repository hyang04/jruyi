...
class Endpoint implements IProducer, IDumpable {
		...
		CLOSED, OPENED {
			...
			public final void send(IMessage message, Endpoint endpoint) {
				...
				if (msg.to() == null) {
					...
					if (entry == null) {
						m_logger.warn(StrUtil.buildString("Route Not Found:", msg));
						msg.close();
						return;
					}
					msg.to(entry.getTo());
	...
	public void onMessage(IMessage message) {
		m_logger.error(StrUtil.buildString(this, " doesn't consume message: ",
				message));
		message.close();
	}
	...
	private boolean onDequeue(IMessage message) {
		...
		try {
			...
			// TODO: notify?
			m_logger.error(
					StrUtil.buildString(this, " unexpected error on pre-handling"),
					e);
			return false;
		}
